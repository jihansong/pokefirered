...
- 숨은 아이템: 호연 플래그 블록(`HOENN_FLAGS_START` 뒤)에 둔다. 맵 데이터의 id 폭을 8→14비트로 넓혔다(ROM 데이터). v0.5.1 전에는 FR/LG 숨은 아이템 범위(191번 뒤)를 썼는데 그 자리가 칸토 관장 격파 플래그와 겹쳐, 104~109번 도로의 숨은 아이템 13개를 주울 수 없었다
...
- 스토리 이벤트(아쿠아·마그마단, 체육관 배지, 전설 포켓몬)는 아직 없다. 맵의 NPC는
  원작의 첫 대사만 말한다.
- 비밀기지, 컨테스트, 배틀 프런티어 기능, 나무열매 재배는 이식하지 않았다. 에메랄드의 나무열매 자리 오브젝트는 v0.5.1에서 지웠다(콜백이 없는 이동 방식이라 그 도로에 들어서면 게임이 리셋됐다).
