...
# Hoenn's hidden items keep their "taken" flag in the Hoenn flag block, not in
# FR/LG's hidden-item range, which runs into FR/LG's own flags past id 190.
...
def main():
    ...
    for m in maps:
        ...
        for o in src.get('object_events', []):
            ...
            # FR/LG has no berry trees (MOVEMENT_TYPE_BERRY_TREE_GROWTH has no callback and
            # resets the game), so Emerald's berry plots are left out
            if o.get('movement_type') == 'MOVEMENT_TYPE_BERRY_TREE_GROWTH': stats['berry_tree'] += 1; continue
    ...
    open(FR + '/include/constants/flags_hoenn.h', 'w').write(
        ...
        + '\n' + ''.join(f'#define {f:<38} (HOENN_FLAGS_START + {len(hoenn_flags) + i})\n'
                         for i, f in enumerate(hidden_flags))
        + f'\n#define HOENN_FLAGS_USED {len(hoenn_flags) + len(hidden_flags)}\n\n#endif  // GUARD_CONSTANTS_FLAGS_HOENN_H\n')
