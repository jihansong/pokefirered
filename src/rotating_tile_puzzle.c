// Ported from pokeemerald for the TRICK HOUSE's rotating tile room (and the
// MOSSDEEP GYM later). Two things differ in FR/LG and are adjusted below:
// the lock/unlock movement actions are named after the facing direction, and
// an object event template keeps movementType inside objUnion.normal.
#include "global.h"
