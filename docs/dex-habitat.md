...
- 도감에 본 것(SEEN) 또는 잡은 것(CAUGHT)으로 등록된 입수 가능한 포켓몬은 분류·키·몸무게·설명을 보여 준다(1~386번 모두 입수 가능하므로 사실상 도감에 등록된 모든 종).
- 특성: 설명 페이지의 몸무게 아래에 `ABILITY` 줄을 넣어 특성 1·2의 이름을 영어 대문자(게임 표기)로 보여 준다.
- 분포 페이지의 타입 아이콘·크기 비교도 같은 조건으로 보여 준다.
